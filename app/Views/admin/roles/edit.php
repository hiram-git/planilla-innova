...
$(document).ready(function() {
    ...
    $("#nameCounter").text($("#name").val().length + "/50");
    $("#descriptionCounter").text($("#description").val().length + "/255");
});

function confirmResetPermissions() {
    Swal.fire({
        title: "¿Resetear permisos?",
        text: "Esta acción eliminará todos los permisos seleccionados",
        icon: "warning",
        ...
        if (result.isConfirmed) {
            $(".perm-checkbox, .select-all, #select-all-permissions, #select-all-read, #select-all-write, #select-all-delete").prop("checked", false);
            toastr.info("Todos los permisos han sido deseleccionados", "Permisos Reseteados");
        }
    });
}
</script>';
    ...
    <div class="col-12">
        <div class="card card-warning">
            <div class="card-header">
                <h3 class="card-title">
                    <i class="fas fa-user-shield"></i> Editar Rol: <?= htmlspecialchars($role['name'] ?? '') ?>
                </h3>
                <?php if (!empty($role['is_admin'])): ?>
                    <div class="card-tools">
                        <span class="badge badge-warning">
                            <i class="fas fa-crown"></i> Rol de Administrador
                        </span>
                    </div>
                <?php endif; ?>
            </div>
            
            <form id="editRoleForm" action="<?= \App\Core\UrlHelper::url('/panel/roles/'.($role['id'] ?? 0).'/update') ?>" method="POST">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4">
                            ...
                            <div class="card card-secondary">
                                ...
                                <div class="card-body">
                                    <div class="row">
                                        <div class="col-6">
                                            <div class="description-block">
                                                <h5 class="description-header"><?= $role['id'] ?? 0 ?></h5>
                                                <span class="description-text">ID del Rol</span>
                                            </div>
                                        </div>
                                        <div class="col-6">
                                            <div class="description-block">
                                                <h5 class="description-header"><?= count($role['permissions'] ?? []) ?></h5>
                                                <span class="description-text">Módulos con Permisos</span>
                                            </div>
                                        </div>
                                    </div>
                                    
                                    <?php if (!empty($role['users_count'])): ?>
                                    <div class="row mt-2">
                                        <div class="col-12">
                                            <div class="description-block">
                                                <h5 class="description-header text-<?= ($role['users_count'] ?? 0) > 0 ? 'primary' : 'muted' ?>">
                                                    <?= $role['users_count'] ?? 0 ?>
                                                </h5>
                                                <span class="description-text">Usuarios Asignados</span>
                                            </div>
                                        </div>
                                    </div>
                                    <?php endif; ?>
                                    ...
                                    <small class="text-muted">
                                        ...
                                        Creado: <?= date('d/m/Y', strtotime($role['created_at'] ?? 'now')) ?>
                                    </small>
                                </div>
                            </div>
                            ...
                            <div class="card card-success">
                                ...
                                <div class="card-body">
                                    <button type="button" class="btn btn-info btn-block" onclick="confirmResetPermissions()">
                                        <i class="fas fa-undo"></i> Resetear Permisos
                                    </button>
                                    <a href="<?= \App\Core\UrlHelper::url('/panel/roles/'.($role['id'] ?? 0)) ?>" class="btn btn-primary btn-block">
                            ...
                            <div class="card card-warning">
                                                    ...
                                                    <td class="text-center">
                                                        <div class="custom-control custom-checkbox">
                                                            <input type="checkbox" 
                                                                   ...
                                                                   value="1"
                                                                   data-module-id="<?= $moduleId ?>"
                                                                   <?= (!empty($role['permissions'][$moduleId]['read'])) ? 'checked' : '' ?>>
                                                    ...
                                                    <td class="text-center">
                                                        <div class="custom-control custom-checkbox">
                                                            <input type="checkbox" 
                                                                   ...
                                                                   value="1"
                                                                   data-module-id="<?= $moduleId ?>"
                                                                   <?= (!empty($role['permissions'][$moduleId]['write'])) ? 'checked' : '' ?>>
                                                    ...
                                                    <td class="text-center">
                                                        <div class="custom-control custom-checkbox">
                                                            <input type="checkbox" 
                                                                   ...
                                                                   value="1"
                                                                   data-module-id="<?= $moduleId ?>"
                                                                   <?= (!empty($role['permissions'][$moduleId]['delete'])) ? 'checked' : '' ?>>
                                ...
                                <div class="card-footer">
                                    <small class="text-muted">
                                        ...
                                        Modifique los permisos según las necesidades del rol.
                                    </small>
                                </div>
                            </div>
                        </div>
                ...
                <div class="card-footer">
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            <a href="<?= \App\Core\UrlHelper::url('/panel/roles/'.($role['id'] ?? 0)) ?>" class="btn btn-info ml-2">
                                <i class="fas fa-eye"></i> Ver Detalles
                            </a>
                        </div>
