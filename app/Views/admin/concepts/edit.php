    ...
    <div class="col-md-12">
        <div class="card">
            <div class="card-header">
                <h3 class="card-title">
                    <i class="fas fa-edit"></i> Editar Concepto de Nómina
                </h3>
            ...
            <form id="conceptForm" action="<?= \App\Core\UrlHelper::concept($concept['id'] . '/update') ?>" method="POST">
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" id="descripcion" name="descripcion" required
                                       value="<?= htmlspecialchars($concept['descripcion'] ?? '') ?>"
                    ...
                    <div class="row">
                            ...
                            <div class="form-group">
                                ...
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="radio" name="tipo_concepto" id="asignacion" value="A" 
                                           <?= ($concept['tipo_concepto'] ?? '') === 'A' ? 'checked' : '' ?>>
                                ...
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="radio" name="tipo_concepto" id="deduccion" value="D"
                                           <?= ($concept['tipo_concepto'] ?? '') === 'D' ? 'checked' : '' ?>>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="radio" name="unidad" id="monto" value="monto" 
                                           <?= ($concept['unidad'] ?? 'monto') === 'monto' ? 'checked' : '' ?>>
                            ...
                            <div class="form-group">
                                ...
                                <div class="input-group">
                                    <textarea class="form-control" id="formula" name="formula" rows="3"
                                              placeholder="Ej: SALARIO * 0.125" required><?= htmlspecialchars($concept['formula'] ?? '') ?></textarea>
                            ...
                            <div class="form-group">
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="checkbox" name="imprime_detalles" id="imprime_detalles" value="1" 
                                           <?= ($concept['imprime_detalles'] ?? 0) ? 'checked' : '' ?>>
                                ...
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="checkbox" name="modifica_valor" id="modifica_valor" value="1" 
                                           <?= ($concept['modifica_valor'] ?? 0) ? 'checked' : '' ?>>
                                ...
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="checkbox" name="valor_referencia" id="valor_referencia" value="1" 
                                           <?= ($concept['valor_referencia'] ?? 0) ? 'checked' : '' ?>>
                                ...
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="checkbox" name="monto_calculo" id="monto_calculo" value="1" 
                                           <?= ($concept['monto_calculo'] ?? 0) ? 'checked' : '' ?>>
                                ...
                                <div class="card card-outline card-primary" style="min-height: 120px;">
                                    <div class="card-body p-2">
                                        <?php if (!empty($tipos_planilla)): ?>
                                            <?php 
                                            // Obtener IDs de tipos de planilla seleccionados
                                            $conceptTipos = [];
                                            if (!empty($concept['tipos_planilla_rel'])) {
                                                foreach ($concept['tipos_planilla_rel'] as $rel) {
                                                    $conceptTipos[] = $rel['id'];
                                                }
                                            }
                                            ?>
                                            <?php foreach ($tipos_planilla as $tipo): ?>
                                                <?php $checked = in_array($tipo['id'], $conceptTipos) ? 'checked' : ''; ?>
                                ...
                                <div class="card card-outline card-success" style="min-height: 120px;">
                                    <div class="card-body p-2">
                                        <?php if (!empty($frecuencias)): ?>
                                            <?php 
                                            // Obtener IDs de frecuencias seleccionadas
                                            $conceptFrecuencias = [];
                                            if (!empty($concept['frecuencias_rel'])) {
                                                foreach ($concept['frecuencias_rel'] as $rel) {
                                                    $conceptFrecuencias[] = $rel['id'];
                                                }
                                            }
                                            ?>
                                            <?php foreach ($frecuencias as $frecuencia): ?>
                                                <?php $checked = in_array($frecuencia['id'], $conceptFrecuencias) ? 'checked' : ''; ?>
                                                <div class="form-check form-check-sm">
                                ...
                                <div class="card card-outline card-warning" style="min-height: 120px;">
                                    <div class="card-body p-2">
                                        <?php if (!empty($situaciones)): ?>
                                            <?php 
                                            // Obtener IDs de situaciones seleccionadas
                                            $conceptSituaciones = [];
                                            if (!empty($concept['situaciones_rel'])) {
                                                foreach ($concept['situaciones_rel'] as $rel) {
                                                    $conceptSituaciones[] = $rel['id'];
                                                }
                                            }
                                            ?>
                                            <?php foreach ($situaciones as $situacion): ?>
                                                <?php $checked = in_array($situacion['id'], $conceptSituaciones) ? 'checked' : ''; ?>
                                                <div class="form-check form-check-sm">
                                            ...
                                            <div class="form-group">
                                                <div class="custom-control custom-switch">
                                                    <input type="checkbox" class="custom-control-input" id="incluir_reporte" 
                                                           name="incluir_reporte" value="1" 
                                                           <?= ($concept['incluir_reporte'] ?? 1) ? 'checked' : '' ?>>
                                        ...
                                        <div class="col-md-4">
                                            <div class="form-group">
                                                ...
                                                <select class="form-control" id="categoria_reporte" name="categoria_reporte">
                                                    <option value="otro" <?= ($concept['categoria_reporte'] ?? 'otro') === 'otro' ? 'selected' : '' ?>>
                                                        Otro
                                                    </option>
                                                    <option value="seguro_social" <?= ($concept['categoria_reporte'] ?? '') === 'seguro_social' ? 'selected' : '' ?>>
                                                        Seguro Social
                                                    </option>
                                                    <option value="seguro_educativo" <?= ($concept['categoria_reporte'] ?? '') === 'seguro_educativo' ? 'selected' : '' ?>>
                                                        Seguro Educativo
                                                    </option>
                                                    <option value="impuesto_renta" <?= ($concept['categoria_reporte'] ?? '') === 'impuesto_renta' ? 'selected' : '' ?>>
                                                        Impuesto sobre la Renta
                                                    </option>
                                                    <option value="otras_deducciones" <?= ($concept['categoria_reporte'] ?? '') === 'otras_deducciones' ? 'selected' : '' ?>>
                                                        Otras Deducciones
                                                    </option>
                                                </select>
                                        ...
                                        <div class="col-md-4">
                                            <div class="form-group">
                                                ...
                                                <input type="number" class="form-control" id="orden_reporte" name="orden_reporte" 
                                                       value="<?= htmlspecialchars($concept['orden_reporte'] ?? 0) ?>"
                                                       min="0" max="999">
...
$(document).ready(function() {
    ...
    $('#conceptForm').on('submit', function(e) {
        ...
        // Validar que no esté vacío tanto fórmula como valor fijo
        var formula = $('#formula').val().trim();
        var valorFijo = $('#valor_fijo').val();

        if (!formula && !valorFijo) {
            e.preventDefault();
            alert('Debe proporcionar al menos una fórmula o un valor fijo');
            return false;
        }
    });
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.results && response.results.length > 0) {
                    // También obtener acumulados existentes del concepto
                    loadExistingAcumulados(<?= $concept['id'] ?>, response.results);
                } else {
    ...
    function loadExistingAcumulados(conceptId, tiposAcumulados) {
        $.ajax({
            url: '<?= url('/panel/concepts/') ?>' + conceptId + '/acumulados',
            method: 'GET',
            dataType: 'json',
            success: function(existingAcumulados) {
                renderAcumuladosForm(tiposAcumulados, existingAcumulados);
            },
            error: function() {
                // Si falla, renderizar sin datos existentes
                renderAcumuladosForm(tiposAcumulados, []);
            }
        });
    }
    
    function renderAcumuladosForm(tiposAcumulados, existingAcumulados = []) {
        let html = '<div class="row">';
        
        // Crear un mapa de acumulados existentes para fácil acceso
        let existingMap = {};
        existingAcumulados.forEach(function(item) {
            existingMap[item.tipo_acumulado_id] = item;
        });
