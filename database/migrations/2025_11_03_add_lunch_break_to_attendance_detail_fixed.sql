-- =====================================================
-- Migración: Agregar Campos de Almuerzo a Detalle de Asistencias (Fixed)
-- Fecha: 2025-11-03
...
SET @drop_lunch_out = IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS
     WHERE TABLE_SCHEMA = 'planilla_prod'
       AND TABLE_NAME = 'attendance_detail'
       AND COLUMN_NAME = 'lunch_out') > 0,
    'ALTER TABLE attendance_detail DROP COLUMN lunch_out',
    'SELECT "Column lunch_out does not exist" as msg'
);
PREPARE stmt FROM @drop_lunch_out;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

SET @drop_lunch_in = IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS
     WHERE TABLE_SCHEMA = 'planilla_prod'
       AND TABLE_NAME = 'attendance_detail'
       AND COLUMN_NAME = 'lunch_in') > 0,
    'ALTER TABLE attendance_detail DROP COLUMN lunch_in',
    'SELECT "Column lunch_in does not exist" as msg'
);
PREPARE stmt FROM @drop_lunch_in;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

SET @drop_scheduled_lunch_out = IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS
     WHERE TABLE_SCHEMA = 'planilla_prod'
       AND TABLE_NAME = 'attendance_detail'
       AND COLUMN_NAME = 'scheduled_lunch_out') > 0,
    'ALTER TABLE attendance_detail DROP COLUMN scheduled_lunch_out',
    'SELECT "Column scheduled_lunch_out does not exist" as msg'
);
PREPARE stmt FROM @drop_scheduled_lunch_out;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

SET @drop_scheduled_lunch_in = IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS
     WHERE TABLE_SCHEMA = 'planilla_prod'
       AND TABLE_NAME = 'attendance_detail'
       AND COLUMN_NAME = 'scheduled_lunch_in') > 0,
    'ALTER TABLE attendance_detail DROP COLUMN scheduled_lunch_in',
    'SELECT "Column scheduled_lunch_in does not exist" as msg'
);
PREPARE stmt FROM @drop_scheduled_lunch_in;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

SET @drop_lunch_duration = IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS
     WHERE TABLE_SCHEMA = 'planilla_prod'
       AND TABLE_NAME = 'attendance_detail'
       AND COLUMN_NAME = 'lunch_duration_minutes') > 0,
    'ALTER TABLE attendance_detail DROP COLUMN lunch_duration_minutes',
    'SELECT "Column lunch_duration_minutes does not exist" as msg'
);
PREPARE stmt FROM @drop_lunch_duration;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

SET @drop_lunch_exceeded = IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS
     WHERE TABLE_SCHEMA = 'planilla_prod'
       AND TABLE_NAME = 'attendance_detail'
       AND COLUMN_NAME = 'lunch_exceeded_minutes') > 0,
    'ALTER TABLE attendance_detail DROP COLUMN lunch_exceeded_minutes',
    'SELECT "Column lunch_exceeded_minutes does not exist" as msg'
);
PREPARE stmt FROM @drop_lunch_exceeded;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- Eliminar índices si existen
SET @drop_idx_lunch_times = IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS
     WHERE TABLE_SCHEMA = 'planilla_prod'
       AND TABLE_NAME = 'attendance_detail'
       AND INDEX_NAME = 'idx_lunch_times') > 0,
    'ALTER TABLE attendance_detail DROP INDEX idx_lunch_times',
    'SELECT "Index idx_lunch_times does not exist" as msg'
);
PREPARE stmt FROM @drop_idx_lunch_times;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

SET @drop_idx_lunch_duration = IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS
     WHERE TABLE_SCHEMA = 'planilla_prod'
       AND TABLE_NAME = 'attendance_detail'
       AND INDEX_NAME = 'idx_lunch_duration') > 0,
    'ALTER TABLE attendance_detail DROP INDEX idx_lunch_duration',
    'SELECT "Index idx_lunch_duration does not exist" as msg'
);
PREPARE stmt FROM @drop_idx_lunch_duration;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- Ahora agregar las columnas
...
CREATE VIEW v_attendance_detail_with_lunch AS
SELECT
    ad.id,
    ad.header_id,
    ad.employee_id,
